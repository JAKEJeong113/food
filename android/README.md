...
└─ ui/
   ...
   └─ screens/
      ├─ auth/                 # 로그인 / 회원가입(새 가구 생성 or 초대코드 합류, 개인정보처리방침 동의)
      ├─ home/                 # 오늘의 추천 Top 3, 가구명 표시, 로그아웃
      ├─ camera/                # 촬영 → 인식 결과 확인/수정 → 반영
      └─ recipes/               # 필터 + 전체 추천 리스트 + "이걸로 먹을래" 재고 차감
```
...
회원가입 화면의 개인정보처리방침 링크는 백엔드의 `/privacy` 페이지를 외부
브라우저로 엽니다(`BuildConfig.API_BASE_URL + "/privacy"`). 서버가
`agreedToPrivacy`가 `true`가 아니면 가입을 거부하므로, 체크박스를 켜기 전에는
회원가입 버튼이 비활성화됩니다.
...
2. ~~로그인/가구(household) 개념 도입~~ — 완료. 이메일+비밀번호 가입, 가구
   ...
   소셜 로그인은 아직 없음.
3. ~~"이걸로 먹을래" → 재고 자동 차감 + 조리 이력~~ — 완료. 재고 부족 파악이
   가능해졌으니 다음은 장보기 리스트다.
...
이번 커밋은 이 로드맵에서 재고 자동 차감까지 마친 단계입니다.
